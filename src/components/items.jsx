    ...
    <table className="itemTable">
        ...
        {props.items &&
          props.items.map((item, index) => (
            <tr key={item.itemID}>
              ...
              <td>
                {props.currFocus == `${item.itemID} description` ? (
                  ...
                  <div
                    ...
                    onClick={() =>
                      props.handleDivClick(`${item.itemID} description`)
                    }
                  >
                    {item.description}
                  </div>
              ...
              <td>
                {props.currFocus == `${item.itemID} quantity` ? (
                  ...
                  <div
                    className="alignCenter"
                    onClick={() =>
                      props.handleDivClick(`${item.itemID} quantity`)
                    }
                  >
                    {item.quantity}
                  </div>
              ...
              <td>
                {props.currFocus == `${item.itemID} unitPrice` ? (
                  ...
                  <div
                    className="alignCenter"
                    onClick={() =>
                      props.handleDivClick(`${item.itemID} unitPrice`)
                    }
                  >
                    {item.unitPrice}
                  </div>
              ...
              <td
                className="alignRight lineTotal"
                type="number"
                name="lineTotal"
              >
              ...
              <td className="alignRight">
                <button
                  ...
                  onClick={(event) =>
                    props.modifyItems("deleteItem", event, item.itemID)
                  }
                >
                  &#x2715;
                </button>
      ...
      <tfoot className="itemsTotal highlight">
        <tr>
          <td colSpan="6">
            <button
              className="btn addNewEntity"
              onClick={() => props.modifyItems("addItem")}
            >
              + Add Item
            </button>
          </td>
        ...
        <tr>
          <td colSpan="4">
            {props.currFocus == "subTotalName" ? (
              <input
                className="inputBox highlight"
                name="subTotalName"
                value={props.data.subTotalName}
                onChange={(event) => props.handleChange(event)}
                maxLength="25"
                onBlur={() => props.handleInputBlur()}
              />
            ) : (
              <div onClick={() => props.handleDivClick("subTotalName")}>
                {props.data.subTotalName}
              </div>
            )}
          </td>
          <td className="alignRight" colSpan="2">
            <b>
              {props.price.currencySymbol} {props.subTotal}
            </b>
          </td>
        </tr>
      </tfoot>
